...
namespace qh
{
    ...
    void ProxyURLExtractor::Extract( const KeyItems& keys, const std::string& raw_url, std::string& sub_url )
    {
        ...
        unsigned pos = raw_url.find('?');
        std::string s = raw_url.substr(pos+1);
        vector<string> v;
        StringSplit(s, '&', 100, v);
        int length = v.size();
        int i = 0;
        for(;i<length;i++){
            std::string work_url = v[i];
            int pos = work_url.find('=');
            if(pos == std::string::npos)
                continue;
            std::string key = work_url.substr(0, pos);
            if(keys.find(key) != keys.end()){
                if(pos < work_url.size()-1 )
                    sub_url = work_url.substr(pos+1, work_url.size());
           }
        }
        v.clear();
#else
